...
func (c *Coordinator) schedule() {
	//handle request from worker, there has two different request:heartbeat and report
	//heartbeat msg: according to different Phase to do different thing
	//  			change the Phase status
	//report msg: just change the report task to finished status
	c.initMapPhase()
	for {
		select {
...
func (c *Coordinator) selectTask(response *HeartbeatResponse) bool {
	//traverse the task slices to select task return to worker
	...
	for id, task := range c.tasks {
		...
		case Working:
			allFinished = false
			if time.Now().Sub(task.startTime) > MaxInterval {
				hasNewJob = true
